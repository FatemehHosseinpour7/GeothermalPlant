...
########### Sensibility ##########################
cprockvect = np.random.randint(SENSIBILITY['A_CPROCK_START'], SENSIBILITY['A_CPROCK_STOP'])
condrockvect = np.random.randint(SENSIBILITY['A_CONDROCK_START']*100, SENSIBILITY['A_CONDROCK_STOP']*100)/100
rhorockvect = np.random.randint(SENSIBILITY['A_RHOROCK_START'], SENSIBILITY['A_RHOROCK_STOP'])

#Definition of Model Function

def Rs_matrix(SENSIBILITY):
    Rs, Rs_max = [], []
    cprockvect = np.random.randint(SENSIBILITY['A_CPROCK_START'], SENSIBILITY['A_CPROCK_STOP'])
    condrockvect = np.random.randint(SENSIBILITY['A_CONDROCK_START']*100, SENSIBILITY['A_CONDROCK_STOP']*100)/100
    rhorockvect = np.random.randint(SENSIBILITY['A_RHOROCK_START'], SENSIBILITY['A_RHOROCK_STOP'])
    alpha = condrockvect/rhorockvect/cprockvect
    # calcolo di come cambia la resistenza del terreno in funzione del tempo
    RS = (1/2/condrockvect)*np.log((2 * np.sqrt(alpha * tt))/rext)
    Rs.append(RS)
...
Rs, Rs_max = Rs_matrix(SENSIBILITY)
...
##DEFINIZIONE GRAFICO
for x in range(10):
    cprockvect = np.random.randint(SENSIBILITY['A_CPROCK_START'], SENSIBILITY['A_CPROCK_STOP']+1)
    condrockvect = np.random.randint(SENSIBILITY['A_CONDROCK_START'], SENSIBILITY['A_CONDROCK_STOP']+1)
    rhorockvect = np.random.randint(SENSIBILITY['A_RHOROCK_START'], SENSIBILITY['A_RHOROCK_STOP']+1)
    print(cprockvect)
    print(condrockvect)
    print(rhorockvect)
    alpha = condrockvect/rhorockvect/cprockvect
