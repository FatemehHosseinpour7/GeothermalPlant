...
def twat_evaluation(zz, step, time, rext, vel, dhydr, portata, cpw, tground, twat_out, L):
    twat_down_set = []
    twat_horizontal_set = []
    twat_in_set = []
    for ii in range(len(zz)):
        ...
        twat_horizontal = (num1 / den1 * funztground(zz[ii]) - (num1 / (2 * den1)-1) * twat_out) / aa #C°
        twat_down = ((twat_horizontal - tground[ii]) * (np.exp((2 * np.pi * condrock * L)/(portata * cpw * np.log((np.pi * (alpha * time)**0.5)/(2 * rext)))))) + tground[ii]
        twat_in = (num1 / den1 * funztground(zz[ii]) - twat_down * aa) / (num1 / (2 * den1)-1) 
        print(f"L: {L}, Depth: {zz[ii]}, twat_out: {twat_out}")
        print(f"twat_horizontal: {twat_horizontal}")
        print(f"twat_down: {twat_down}")
        print(f"twat_in: {twat_in}")
        print()
        twat_horizontal_set.append(twat_horizontal)
        twat_down_set.append(twat_down)
        twat_in_set.append(twat_in)
    ...
    return twat_in_set, twat_down_set, twat_horizontal_set
...
def plot_2(twat_out, twat_in_set, twat_down_set, twat_horizontal_set, tground, zz, step):
    ...
    for i in range(len(zz)):
        ax.plot([twat_out, twat_horizontal_set[i]], [0, zz[i]], 'r', label='T upward water')
        ax.plot([twat_horizontal_set[i], twat_down_set[i]], [zz[i], zz[i]],'y', label='T lateral water')
        ax.plot([twat_down_set[i], twat_in_set[i]], [zz[i], 0],'b', label='T downward water')
        if i == 0:
...
for step in t: 
    twat_in_set, twat_down_set, twat_horizontal_set = twat_evaluation(zz, step, time, rext, vel, dhydr, portata, cpw, tground, twat_out, L)
    plot_2(twat_out, twat_in_set, twat_down_set, twat_horizontal_set, tground, zz, step)
